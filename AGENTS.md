...
- **Dependency direction is one-way**: `editors → server → core`. `packages/core`
...
- **The catalog is the single source of truth.** A diagnostic's severity, message
  and href are written once in `packages/core/src/semantic/catalog.ts` and reach a
...
- **`docs/reference/` is generated.** It is projected from the built core by
...
| Working on…                                   | Read                                      |
| --------------------------------------------- | ----------------------------------------- |
| Layers, core's directories, the launchers     | `docs/01-architecture.md`                 |
...
- **Never edit a generated file.** `docs/reference/*` and
  `packages/core/src/schema/hoverfly.schema.generated.ts` come from their
