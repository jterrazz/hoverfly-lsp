...
describe('fourslash harness — parseMarkedDocument', () => {
    test('strips a default marker and records its position', () => {
    ...
    test('supports multiple named markers in one document', () => {
        ...
        expect(positions.has('first')).toBeTruthy();
        expect(positions.has('second')).toBeTruthy();
    });

    test('works for a bare (unquoted) value position', () => {
        ...
        expect(text).toBe(`{"matcher": }`);
        expect(positions.has('bare')).toBeTruthy();
    });

    test('throws on a duplicate marker name', () => {
        // Given - the same name twice
        const source = `{"a":"⟦x⟧","b":"⟦x⟧"}`;
        // Then - parsing rejects the ambiguity
        expect(() => parseMarkedDocument(source)).toThrow(/Duplicate/u);
    });
    ...
    test('throws on unbalanced brackets', () => {
        ...
        expect(() => parseMarkedDocument(source)).toThrow(/Unbalanced/u);
    });
});
