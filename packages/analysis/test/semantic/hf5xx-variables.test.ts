...
describe('hF510 / HF511 — variable function precedence', () => {
    test('hF510 for a raymond block built-in, never HF511', () => {
        ...
        expect(codes(diags)).toStrictEqual(['HF510']);
        expect(diags[0]?.message).toContain('block built-ins');
    });

    test('hF511 for an otherwise-unknown function, never HF510', () => {
    ...
    test('accepts a valid Hoverfly helper function (no HF510/HF511)', () => {
    ...
    test("ignores a missing/non-string function (schema's concern)", () => {
...
describe("hF512 — argument arity vs the helper's HelperSpec", () => {
    test('flags too few args for a fixed-arity helper', () => {
        ...
        expect(codes(diags)).toStrictEqual(['HF512']);
    ...
    test('flags too many args for a fixed-arity helper', () => {
    ...
    test('treats missing arguments as zero args', () => {
    ...
    test('enforces only a minimum for variadic helpers', () => {
    ...
    test('does NOT arity-check an unknown function (HF511 owns that)', () => {
...
describe('hF214 — templatable name charset', () => {
    ...
    test('accepts a word-char-only name on both variables and literals', () => {
        // Given - clean names
        const diags = runData({
...
describe('hF213 — destination full URL', () => {
    test('flags an exact destination value containing a scheme', () => {
        ...
        expect(codes(diags)).toStrictEqual(['HF213']);
        expect(diags[0]?.message).toContain('http://api.example.com/v1');
    });

    test('flags a default (no matcher) destination value containing ://', () => {
    ...
    test('does NOT flag a glob/regex destination (slashes may be intentional)', () => {
    ...
    test('does NOT flag a bare host[:port] exact destination', () => {
    ...
    test('does NOT flag :// on a non-destination field (e.g. path)', () => {
...
describe('a valid variables+literals+destination block stays silent', () => {
    test('correct faker/requestBody arities, clean names, host:port destination', () => {
