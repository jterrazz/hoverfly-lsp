...
describe('runRules', () => {
    test('concatenates rule output', () => {
        ...
        // Then - both appear
        expect(out.map((d) => d.code)).toStrictEqual(['HF103', 'HF104']);
    });

    test('swallows a throwing rule rather than failing the pass', () => {
        ...
        expect(out.map((d) => d.code)).toStrictEqual(['HF104']);
    });
});

describe('applyHF102Layer', () => {
    test('re-tags every schema diagnostic as HF102 with hoverfly source', () => {
    ...
    test('suppresses a schema diagnostic overlapping an HF2xx semantic diagnostic', () => {
    ...
    test('keeps a schema diagnostic that overlaps a non-suppressing semantic diagnostic', () => {
...
describe('sortByRange', () => {
    test('orders by start line, then character, then end', () => {
        ...
        expect(out).toStrictEqual(['z', 'a', 'b', 'c']);
    });
});
