...
| `randomString` | inline | — | `{{randomString}}` | Random string. |
| `randomStringLength` | inline | length: int | `{{randomStringLength 10}}` | Random string of the given length. |
| `randomBoolean` | inline | — | `{{randomBoolean}}` | Random boolean as the string "true"/"false". |
| `randomInteger` | inline | — | `{{randomInteger}}` | Random integer. |
| `randomIntegerRange` | inline | min: int, max: int | `{{randomIntegerRange 1 10}}` | Random integer in [min, max]. |
| `randomFloat` | inline | — | `{{randomFloat}}` | Random float. |
| `randomFloatRange` | inline | min: float, max: float | `{{randomFloatRange 1.0 2.0}}` | Random float in [min, max]. |
| `randomEmail` | inline | — | `{{randomEmail}}` | Random email (powered by icrowley/fake, NOT gofakeit). |
| `randomIPv4` | inline | — | `{{randomIPv4}}` | Random IPv4 address. |
| `randomIPv6` | inline | — | `{{randomIPv6}}` | Random IPv6 address. |
| `randomUuid` | inline | — | `{{randomUuid}}` | Random UUID. |
...
| `concat` | inline | — | `{{concat 'a' 'b' (getArray 'x')}}` | Concatenate all arguments into one string; flattens any []interface{} argument. |
